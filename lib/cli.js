...
function jshint (args, options) {
    var dir = path.parse(require.resolve('jshint/package.json')).dir;
    var cmd = path.resolve(dir, '..', '.bin', 'jshint') + ' ' + args;
...
function jshintCallback (cmd, options, err, stdout, stderr) {
    ...
    if (isRuntimeError(err)) {
        ...
        printErrorAndExit(stderr || err.message, err.code);
    }

    try {
        report = JSON.parse(stdout.toString());
    } catch (e) {
        handleRuntimeError(e, err, stderr, stdout);
    }
...
function handleRuntimeError (e, err, stderr, stdout) {
